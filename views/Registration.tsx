...
export const Registration: React.FC<RegistrationProps> = ({ onRegister, t, currentTemple }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Fix: Adding missing properties auraPoints, badges, and completedQuests to match the Pilgrim interface
    const newPilgrim: Pilgrim = {
      ...
      colorCode: colorMap[formData.slotTime] || 'GREEN',
      qrValue: `BHR-${Date.now()}`,
      status: 'PENDING',
      auraPoints: 0,
      badges: [],
      completedQuests: []
    ...
    setFormData({ name: '', age: '', gender: 'Male', groupSize: '1', slotTime: '08:00 AM' });
  };
      ...
      <div className="bg-white dark:bg-slate-900 p-8 rounded-[2rem] shadow-xl dark:shadow-none border border-slate-100 dark:border-white/5 flex flex-col h-full transition-colors duration-300">
        <h3 className="text-2xl font-black mb-8 flex items-center gap-3 dark:text-white">
          ...
          <div className="flex flex-col">
            <span className="leading-tight">{t('registration')}</span>
            <span className="text-xs text-slate-400 dark:text-slate-500 font-bold uppercase tracking-widest">{currentTemple?.name} Center</span>
          </div>
        </h3>
              ...
              <div className="bg-white dark:bg-slate-900 rounded-[2.4rem] p-8 overflow-hidden relative transition-colors duration-300">
                ...
                <div className="grid grid-cols-3 gap-4 mb-10 relative z-10">
                  ...
                  <div className="bg-slate-50/80 dark:bg-white/5 backdrop-blur-sm p-4 rounded-2xl border border-slate-100 dark:border-white/5">
                    ...
                    <p className="font-black truncate" style={{ color: themePrimary }}>NORTH-A</p>
                  </div>
                </div>
                <div className="bg-white dark:bg-white/90 p-6 rounded-3xl border-2 border-dashed border-slate-200 dark:border-white/20 flex flex-col items-center relative z-10">
                  <img src={`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${lastRegistered.qrValue}`} alt="QR" className="w-32 h-32" />
