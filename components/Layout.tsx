...
export const Layout: React.FC<LayoutProps> = ({
  ...
  const allTabs = [
    ...
    { id: 'biometric-station', label: 'Iris Station', icon: 'fa-eye', roles: ['ADMIN', 'BIOMETRIC_OFFICER'] },
    ...
    { id: 'scarf-lifecycle', label: 'Scarf Lifecycle', icon: 'fa-recycle', roles: ['ADMIN'] },
    { id: 'system-architecture', label: 'System Arch.', icon: 'fa-server', roles: ['ADMIN'] },
    ...
    <div className="min-h-screen flex flex-col md:flex-row relative">
      <aside className="w-full md:w-72 bg-slate-900 dark:bg-black text-white flex-shrink-0 z-20 flex flex-col transition-colors duration-300">
        <div className="p-8 border-b border-white/10 flex items-center gap-4">
          <div className="w-12 h-12 rounded-2xl flex items-center justify-center shadow-lg transition-transform hover:rotate-12 duration-500" style={{ backgroundColor: currentTemple.themeColor }}>
          ...
          <nav className="space-y-2">
            {visibleTabs.map((tab) => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                className={`w-full flex items-center gap-4 px-4 py-4 rounded-2xl transition-all relative group overflow-hidden ${activeTab === tab.id ? 'text-white' : 'text-white/40 hover:bg-white/5 hover:text-white'}`}>
                {activeTab === tab.id && (<div className="absolute inset-0 z-0 opacity-100" style={{ backgroundColor: currentTemple.themeColor }}></div>)}
        ...
        <div className="p-6 border-t border-white/10 space-y-3">
          <div className="flex items-center gap-2 px-4 py-2 bg-white/5 rounded-xl">
            ...
            <span className="text-[9px] font-black text-white/40 uppercase tracking-widest">System Online</span>
          </div>
          <button onClick={onLogout} className="w-full flex items-center gap-4 px-4 py-3 rounded-xl text-white/40 hover:text-white hover:bg-white/5 transition-all text-sm font-bold">
            <i className="fas fa-sign-out-alt w-5"></i><span>Switch Role</span>
          </button>
        </div>
      </aside>
      ...
      <main className="flex-1 overflow-auto flex flex-col h-screen bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
        <header className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-slate-200 dark:border-white/5 px-8 py-4 sticky top-0 z-30 flex justify-between items-center shadow-sm">
          ...
          <div className="flex items-center gap-4">
            <button onClick={toggleDarkMode} className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-all shadow-sm border border-slate-200 dark:border-white/5">
            ...
            <div className="bg-slate-100 dark:bg-slate-800 p-1 rounded-xl flex gap-1 border border-slate-200 dark:border-white/5">
              {(['EN', 'HI', 'TE'] as Language[]).map(l => (
                <button key={l} onClick={() => setLanguage(l)} className={`px-3 py-1.5 rounded-lg text-xs font-black transition-all ${language === l ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm' : 'text-slate-400 dark:text-slate-500'}`}>{l}</button>
              ))}
            </div>
          </div>
        </header>
